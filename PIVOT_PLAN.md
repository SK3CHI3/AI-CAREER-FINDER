...
| No school/teacher | School onboarding, teacher management; parents share student dashboard |
...
- No parent-specific role (parents share the student dashboard with the student).
...
| **student** | Learner in a school | View own competency-based career insights, AI chat, grades (teacher-verified where applicable). **Parents share this dashboard** (no separate parent role). |
...
│  • Student dashboard (shared with parents – no separate parent role)    │
└─────────────────────────────────────────────────────────────────────────┘
```
...
| **grade_sources** | Column on student_grades: `student_self` \| `teacher_upload` \| `teacher_edit` (for verified vs self-reported). |
...
| Table | Changes |
|-------|---------|
| **profiles** | Add optional `school_id` (for students/teachers); extend `role` = student \| admin \| school \| teacher (no parent role – parents share student dashboard). Keep backward compatibility during migration. |
...
| **Student dashboard** | Single student view | Keep; add “verified by school” badges. **Parents share the student dashboard** (same login or shared access – no separate parent role). |
...
- **DB schema design** – Finalize new tables (schools, school_members, classes, class_enrollments, school_subscriptions) and extended student_grades (source, etc.). No parent role or parent_students table – parents share student dashboard.
- **RLS & policies** – School-scoped access: teachers see only their school/classes; school admin sees school; students see self (+ school context).
- **Role enum** – Extend from `student \| admin` to `student \| admin \| school \| teacher` in DB and app (profiles.role). No parent role.
...
### Phase 3: Tiered school subscriptions
...
- **Parents** – No separate role or dashboard; parents share the student dashboard (same account or shared access as decided in UX).

**Deliverables:** School subscription tiers and payment; admin school list and subscription oversight.
...
- **Routing:** `App.tsx` – add routes for `/school`, `/teacher`; extend redirect logic in Auth/Dashboard by role. No `/parent` – parents share student dashboard.
- **Lib:** `supabase.ts`, `session-utils.ts`, `auth-utils.ts` – keep; add school/teacher in RLS.
...
(No parent dashboard or parent-service – parents share the student dashboard.)

### 5.3 Shared types
- **`src/types/supabase.ts`** (or generated from DB) – Add schools, school_members, classes, class_enrollments, school_subscriptions, and extended student_grades (source, upload_batch_id, etc.). No parent_students.
